...
export default class TotalIncome extends React.Component<TotalIncomeProp, TotalIncomeState> {
  ...
  //     this.setState({ code: nextProps.code }, this.loadData);
  //   }
  // }
  loadData() {
    ...
    Api.getTotalIncome({ code: this.props.code }).then(res => {
      console.log('res', res);
      const { data } = res;
      ...
      const totalIncomes = tableJson.map((t: { [s: string]: unknown; } | ArrayLike<unknown>) => {
        return Object.values(t);
      });
      const xAxisData = totalIncomes.map((k: any[]) => k[0]).reverse();
      const seriesData = totalIncomes.map((k: string[]) => parseInt((k[1] + '').replace(/,/g, ''), 10)).reverse();
      ...
      this.setState({ option, totalIncomes });
    }).finally(() => {
      this.setState({ isLoading: false });
    });
  }
  render() {
    const { option, isLoading, totalIncomes } = this.state;
    return (
      <Spin spinning={isLoading}>
        {totalIncomes && totalIncomes.length > 0 && <EchartsReact option={option} />}
      </Spin>
    );
  }
}
